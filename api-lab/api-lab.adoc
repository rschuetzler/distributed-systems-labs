...
+
NOTE: Notice that changing the parameters also changes the URL in the bar above. It adds `?count=5` after the URL that you previously put. That is how parameters are passed in HTTP GET requests. Postman makes it easier to set different parameters by presenting them in a table instead of in a long string.
